...
function getAlbum(req,res){
   ...
   //res.status(200).send({message: 'Metodo getAlbum del controlador Album.js'});
}

function getAlbums(req,res){
    var artistId = req.params.artist;
    if(!artistId){
        //Get all the albums form the database
        var find = Album.find({}).sort('title');
    }else{
        //Get all the albums from the artist form de database
...
function saveAlbum(req,res){
    ...
    album.save((err,albumStored) => {
        if(err){
            ...
            if(!albumStored){
                res.status(404).send({message: 'Not saved the album'});
            }else{
